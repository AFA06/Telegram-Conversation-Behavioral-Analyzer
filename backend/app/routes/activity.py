...
@router.get("/hourly")
def hourly(role: str = RoleParam, db: Session = Depends(get_db)) -> list[dict]:
    return activity_analyzer.hourly_activity(messages_dataframe(db), role)


@router.get("/weekday")
def weekday(role: str = RoleParam, db: Session = Depends(get_db)) -> list[dict]:
    return activity_analyzer.weekday_activity(messages_dataframe(db), role)


@router.get("/heatmap")
def heatmap(role: str = RoleParam, db: Session = Depends(get_db)) -> dict:
    return activity_analyzer.heatmap(messages_dataframe(db), role)


@router.get("/daily")
def daily(year: int = Query(...), month: int = Query(..., ge=1, le=12), db: Session = Depends(get_db)) -> list[dict]:
    return activity_analyzer.daily_activity(messages_dataframe(db), year, month)


@router.get("/available-months")
def available_months(db: Session = Depends(get_db)) -> list[dict]:
